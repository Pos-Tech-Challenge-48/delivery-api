...
func (r *OrderRepository) Save(ctx context.Context, order *domain.Order) error {
	query := `
	INSERT INTO restaurant_order (restaurant_order_id, restaurant_order_customer_id, restaurant_order_status_id)
	VALUES ($1, $2, (SELECT status_id from Status where status_name = $3))
`
	_, err := r.db.Exec(
		query,
		order.ID,
